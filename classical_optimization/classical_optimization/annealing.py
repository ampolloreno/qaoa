...
class StrategyChain:
    ...
    def __init__(self, acceptance_param, visit_dist, func_wrapper,
                 minimizer_wrapper, rand_gen, energy_state, sigma):
    ...
    def accept_reject_gelfand(self, j, e, x_visit):
        r = self._rand_gen.uniform()
        import scipy.special.erf as erf
        sigma = self.sigma
        x = e - self.energy_state.current_energy
        exp_temp = -np.exp(x/self.temperature_step)\
        *(np.exp(sigma/(4*self.temperature_step^2))/2*self.temperature_step)*\
        (np.sqrt(np.pi)*(sigma - sigma*erf(x/sigma - sigma/(2*self.temperature_step)))
         + 2*self.temperature_step*np.exp(-(sigma**2-2*x*self.temperature_step)**2/(4*sigma**2*self.temperature_step**2)))\
        + sigma*np.sqrt(np.pi)/2*(1 + erf(x/sigma))
        pqv_temp = 1.0 - ((1.0 - self.acceptance_param) * np.log(exp_temp))
        if pqv_temp <= 0.:
            pqv = 0.
        else:
            pqv = np.exp(np.log(pqv_temp) / (
    ...
    def run(self, step, temperature):
        ...
        for j in range(self.energy_state.current_location.size * 2):
            ...
            if e < self.energy_state.current_energy:
                ...
                self.accept_reject_gelfand(j, e, x_visit)
...
def dual_annealing(func, bounds, args=(), maxiter=1000,
                   ...
                   callback=None, x0=None, local_search_options=None, sigma=0.):
    """
    ...
    strategy_chain = StrategyChain(accept, visit_dist, func_wrapper,
                                   minimizer_wrapper, rand_state, energy_state, sigma)
    need_to_stop = False
    iteration = 0
    message = []
